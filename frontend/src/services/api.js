...
export const loginUser = async (email, password) => {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      err.detail || "Authentication failed. Please check credentials."
    );
  }
...
// ============================================================
// CLINICAL REPORT — AUTHORITATIVE SCREENING REPORT
// ============================================================

export const fetchClinicalReport = async (screeningId) => {
  if (!screeningId) {
    throw new Error("Screening ID is required to load the clinical report.");
  }

  const response = await fetch(
    `${API_BASE_URL}/screenings/${screeningId}/report`,
    {
      method: "GET",
      headers: {
        Accept: "application/json",
  ...
  if (!response.ok) {
    ...
    throw new Error(
      err.detail || "Failed to fetch clinical report."
    );
  }
...
export const verifyScreening = async (screeningId, decision, notes) => {
  const response = await fetch(
    `${API_BASE_URL}/screenings/${screeningId}/verify`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    ...
    throw new Error(
      err.detail || "Doctor verification failed."
    );
  }
...
// ============================================================
// DIRECT AI INFERENCE
// (AUTHENTICATED & BACKWARDS COMPATIBLE)
// ============================================================
...
export const generateClinicalReport = async (
  ...
  const payload = {
    patient_info: {
      ...
      examined_eye:
        patientInfo.examined_eye ||
        "OD - Right Eye",

      diabetes_type:
        patientInfo.diabetes_status ||
        "Type 2",

      duration_years: patientInfo.diabetes_duration
        ? parseInt(
            String(patientInfo.diabetes_duration).replace(/\D/g, "") ||
              "8",
            10
          )
        : 8,
      ...
      clinician_notes:
        patientInfo.medical_notes ||
        "Automated preliminary screening via NetraScan AI.",
    },
